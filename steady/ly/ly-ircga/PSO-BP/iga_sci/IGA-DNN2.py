...
from iga_sci.DNN import fitModel
from iga_sci.DNN import evaluate
...
def run():
    ...
    runTime = end - start
    save_result_txt('./experiment_result/IGA_sci实验2结果' + str(int(dt.datetime.now().timestamp())) + '.txt',
                    'MAE结果：' + str(gbest_mae) + '\n' +
                    'MSE变化：' + str(gbest) +
