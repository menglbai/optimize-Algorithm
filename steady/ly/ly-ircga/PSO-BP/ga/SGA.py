...
class GA:
    ...
    def __init__(self, parameter, func):
        ...
        for i in range(self.parameter[3]):
            ...
            mse, mae = self.evaluate(geneinfo)  # evaluate each chromosome
            pop.append( {'Gene': Gene(data=geneinfo), 'fitness': mse, 'mae': mae})  # store the chromosome and its fitness
    ...
    def roulette_wheel_selection(self, population, fitness):
        ...
        for i in range(len(cum_probabilities)):
            if random_num <= cum_probabilities[i]:
                return population[i]

    # 选择算子
    def selection(self, individuals, k):
        chosen = []
        fitness = [individuals[i].get('fitness') for i in range(len(individuals))]
        for i in range(k):
            chosen.append(self.roulette_wheel_selection(individuals, fitness))
        return chosen
    ...
    # # 变异算子
    # def mutation(self, crossoff, bound, t):
    #
    #     dim = len(crossoff.data)
    #
    #     if dim == 1:
    ...
    #     if r >= 0.5:
    #         # 向上变异
    #         crossoff.data[pos] = crossoff.data[pos] + (bound[1][pos] - crossoff.data[pos]) * (1 - t / self.parameter[2])
    #     else:
    #         # 向下变异
    #         crossoff.data[pos] = crossoff.data[pos] + (bound[0][pos] - crossoff.data[pos]) * (1 - t / self.parameter[2])
    #     return crossoff
...
def SGA(func, cxpb, mutpb, ngen, popsize, up, low):
    parameter = [cxpb, mutpb, ngen, popsize, low, up]
    ga = GA(parameter, func)
    ga.run()
    print('gbest', ga.gbest)
    print('gbest_mae', ga.gbest_mae)
    plt.plot(ga.gbest)
    plt.plot(ga.gbest_mae)
    plt.show()
    return ga.gbest,ga.gbest_mae
...
if __name__ == "__main__":
    ...
    # plt.plot(iga.gbest)
    # plt.show()
    SGA(func=evaluate, cxpb=0.8, mutpb=0.1, ngen=500, popsize=20, up=[30, 30, 30, 30], low=[1, 1, 1, 1])
