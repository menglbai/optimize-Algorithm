% DA算法主程序
clear
close
clc
R=30;% 实验次数
test.sol=[];
test.res=[];
Result_DA=repmat(test,R,1);
cg.cg=[];
cg_DA=repmat(cg,R,1);
SearchAgents_num=30; % 蜻蜓数量
Function_name='F'; % Name of the test function
Max_iteration=1000; % 最大迭代次数
for r=1:R
tic
[lb,ub,dim,fobj]=Get_Functions_details(Function_name);%函数相关参数初始化
[Best_score,Best_pos,cg_curve]=DA(SearchAgents_num,Max_iteration,lb,ub,dim,fobj);
display(['The best solution obtained by DA is : ', num2str(Best_pos')]);
display(['The best optimal value of the objective funciton found by DA is : ', num2str(Best_score)]);  
toc
Result_DA(r).res=Best_score;
Result_DA(r).sol=Best_pos;
cg_DA(r).cg=cg_curve;
end
Best_DA=Result_DA(1).res;
for i=2:R
    if Result_DA(i).res<Best_DA
        Best_DA=Result_DA(i).res;
    end
end
Best_DA
Worst_DA=Result_DA(1).res;
for i=2:R
    if Result_DA(i).res>Worst_DA
        Worst_DA=Result_DA(i).res;
    end
end
Worst_DA
s=0;
for k=1:R
    s=s+Result_DA(k).res;
end
Mean_DA=s/R
o=0;
for ii=1:R
    o=o+(Result_DA(ii).res-Mean_DA)^2;
end
Std_DA=sqrt(o/(R-1))
C=zeros(1,Max_iteration);
for c=1:R
    C=C+cg_DA(c).cg;
end
CG_DA=C./R;
% %函数图像
% figure('Position',[400 400 560 190])
% %Draw search space
% subplot(1,2,1);
% func_plot(Function_name);
% title('Test function')
% xlabel('x_1');
% ylabel('x_2');
% zlabel([Function_name,'( x_1 , x_2 )'])
% grid off
% %Draw objective space
% subplot(1,2,2);
% semilogy(cg_curve,'Color','r')
% title('Convergence curve')
% xlabel('Iteration');
% ylabel('Best score obtained so far');
% axis tight
% grid off
% box on
% legend('DA')