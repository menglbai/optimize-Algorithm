...
figure(2)
G;
TPDA=[0.4072 0.3076 0.4203 0.3692 0.3128 0.4164 0.4114];
Y=[];
for i=1:7
    Y(i,1)=G(i);
    Y(i,2)=TPDA(i);
...
h=bar(X,Y,1);      
 %修改横坐标名称、字体
set(gca,'XTickLabel',{'目标\newline类型','径向\newline速度','舰艇\newline重要性','无人机\newline数量','目标\newline距离','目标\newline高度','目标\newline方位角'},'FontSize',12,'FontName','宋体');
...
% ITOPSIS结果：[0.9757 0.0037 0.9662 0.1735 0.3035 0.7341]
% TPDA结果：
ITOPSIS=[0.9757 0.0037 0.9662 0.1735 0.3035 0.7341];
TPDA_AA=[2.3247 0.0462 2.2690 0.8269 1.2422 2.0437];
figure(3)
YY=[];
for i=1:6
    YY(i,1)=AA(i); 
...
h=bar(X,YY,1);      
%修改横坐标名称、字体
set(gca,'XTickLabel',{'A','B','C','D','E','F'},'FontSize',12,'FontName','Times New Roman');
% 设置柱子颜色,颜色为RGB三原色，每个值在0~1之间即可
set(h(1),'FaceColor',[30,150,252]/255)     
set(h(2),'FaceColor',[162,214,249]/255)    
...
%修改图例
legend({'\fontname{Times New Roman}PSO','\fontname{Times New Roman}TPDA','\fontname{Times New Roman}ITOPSIS'},'FontSize',12);