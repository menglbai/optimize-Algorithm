...
%     yy=zeros(1,m);
%     for j=1:m
%         xx(j)=x(i)*j/(m+1)+x(i+1)*(m+1-j)/(m+1);
%         yy(j)=y(i)*j/(m+1)+y(i+1)*(m+1-j)/(m+1);
%     end
%     for jj=1:m
        for k=1:max(size(a))
...
% F_3D
function Length=F_3D(X)
S=[1 1 1];
T=[100 100 40];
col=max(size(X));
dim=col/3;
for j=1:dim
    dragonfly(j).pos.x=X(3*j-2);
    dragonfly(j).pos.y=X(3*j-1);
    dragonfly(j).pos.z=X(3*j);
end
%求航迹总长度
Length=sqrt((S(1)-X(1))^2+(S(2)-X(2))^2+(S(3)-X(3))^2)+sqrt((T(1)-X(col-2))^2+(T(2)-X(col-1))^2+(T(3)-X(col))^2);
for i=1:dim-1
    Length=Length+sqrt((X(3*i-2)-X(3*i+1))^2+(X(3*i-1)-X(3*i+2))^2+(X(3*i)-X(3*i+3))^2);
end
% 定义山峰地图
mapRange = [100,100,100];% 地图长、宽、高范围
[X,Y,Z] = defMap(mapRange);
[flag,fitness,path] = calFitness(S,T,X,Y,Z,dragonfly(i).pos);
if flag==1
    Length=1000*Length;
end
end