...
Function_name='F_3D'; % Name of the test function
for T=1:R
tic
% Load details of the selected benchmark function
[lb,ub,dim,fobj]=Get_Functions_details(Function_name);%函数相关参数初始化
[Best_score,Best_pos,cg_curve]=TPDA_3D(SearchAgents_num,Max_iteration,lb,ub,dim,fobj);
...
toc
Result_TPDA(T).res=Best_score;
Result_TPDA(T).sol=Best_pos;
cg_TPDA(T).cg=cg_curve;
end
