...
if __name__ == '__main__':
    ...
    axes2.set_title('(b)DE法', y=-0.25, fontsize='small', fontproperties=font_set)
    axes2.set_ylabel('震塌比例', fontproperties=font_set)
    axes2.set_xlabel('测试样本序列', fontproperties=font_set)
    line = axes2.plot(x, de_dnn_data, line_types[0], lw=2, label="DE-DNN", color=colors[0])
    # line1 = axes2.plot(x, target, line_types[1], lw=2, label="True", color=colors[1])
    points = axes2.plot(target, 'o', markersize=8, c='k', label="True")
    plt.legend(loc=0, prop={'size': 16}, ncol=1)



    # 子图3  pso-dnn
    axes4 = plt.subplot(grid[0, 2])
    axes4.set_title('(c)PSO法', y=-0.25, fontsize='small', fontproperties=font_set)
    axes4.set_ylabel('震塌比例', fontproperties=font_set)
    axes4.set_xlabel('测试样本序列', fontproperties=font_set)
    line = axes4.plot(x, pso_dnn_data, line_types[0], lw=2, label="pso-DNN", color=colors[0])
    # line1 = axes4.plot(x, target, line_types[1], lw=2, label="True", color=colors[1])
    points = axes4.plot(target, 'o', markersize=8, c='k', label="True")
    plt.legend(loc=0, prop={'size': 16}, ncol=1)
    ...
    axes3.set_title('(d)GA法', y=-0.25, fontsize='small', fontproperties=font_set)
    axes3.set_ylabel('震塌比例', fontproperties=font_set)
    axes3.set_xlabel('测试样本序列', fontproperties=font_set)
    line = axes3.plot(x, ga_dnn_data, line_types[0], lw=2, label="GA-DNN", color=colors[0])
    # line1 = axes3.plot(x, target, line_types[1], lw=2, label="True", color=colors[1])
    points = axes3.plot(target, 'o', markersize=8, c='k', label="True")
    plt.legend(loc=0, prop={'size': 16}, ncol=1)

    # 子图5  iga-dnn
    axes3 = plt.subplot(grid[1, 1])
    axes3.set_title('(e)IGA法', y=-0.25, fontsize='small', fontproperties=font_set)
    axes3.set_ylabel('震塌比例', fontproperties=font_set)
    axes3.set_xlabel('测试样本序列', fontproperties=font_set)
    line = axes3.plot(x, iga_dnn_data, line_types[0], lw=2, label="IGA-DNN", color=colors[0])
    line1 = axes3.plot(x, target, line_types[1], lw=2, label="True", color=colors[1])
    # points = axes3.plot(target, 'o', markersize=8, c='k', label="True")
    plt.legend(loc=0, prop={'size': 16}, ncol=1)

    path_name = "comparion_model_predict"
